...
**実装内容**:
1. Service Worker実装の検討
2. React Queryによるデータキャッシング
3. Static Assets のブラウザキャッシュ設定
4. stale-while-revalidate戦略
...
for (const record of records) {
  record.weather = await WeatherModel.findByRecordId(record.id);
}
...
### 15.4 パフォーマンス計測とモニタリング

#### 15.4.1 Lighthouse CI統合
...
| Week | タスク | 優先度 | 期待効果 |
|------|--------|--------|----------|
| Week 1 | コード分割実装 | 高 | バンドルサイズ40%削減 |
| Week 1 | データベースインデックス追加 | 高 | クエリ70%高速化 |
| Week 2 | APIページネーション実装 | 高 | レスポンス60%削減 |
| Week 2 | gzip圧縮有効化 | 高 | 転送量50%削減 |
| Week 2 | 画像最適化 | 中 | LCP40%改善 |
| Week 2 | Lighthouse CI統合 | 中 | 継続的監視 |
| Week 3 | React Query導入 | 中 | キャッシュ効率化 |
| Week 3 | パフォーマンスダッシュボード | 低 | 可視化 |
